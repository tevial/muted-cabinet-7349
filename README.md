...
Keyboard editing works on the selected caption group: `Space` toggles loop playback, `Tab` selects the next group, `Shift+Tab` selects the previous group, `A`/`D` move the start boundary by one frame, and `ArrowLeft`/`ArrowRight` move the end boundary by one frame.
