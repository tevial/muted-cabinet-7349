...
| API runtime | FastAPI + Uvicorn + Stable-ts/OpenAI adapters | Needed wherever provider secrets or local model runtimes live. |
...
| Local Stable-ts model runtime | First transcription can be slow while the model downloads; CPU-only inference can be slower than OpenAI | Keep provider adapter isolated and retain OpenAI fallback in `auto` | API server |
